  ...
  if (collapsed) {
    return (
      <div className="w-14 bg-gradient-to-b from-gray-50 to-white border-r border-gray-200 flex flex-col shadow-sm">
        ...
        {tabs.map((tab) => {
          const Icon = tab.icon
          const SolidIcon = tab.id === 'thumbnails' ? PhotoSolidIcon :
                           tab.id === 'bookmarks' ? BookmarkSolidIcon :
                           tab.id === 'annotations' ? ChatBubbleLeftSolidIcon :
                           tab.id === 'layers' ? RectangleStackSolidIcon :
                           PaperClipSolidIcon
  ...
  return (
    <div className="w-80 bg-gradient-to-b from-gray-50 to-white border-r border-gray-200 flex flex-col shadow-sm">
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        {uiState.sidebarTab === 'bookmarks' && (
          <BookmarksPanel />
        )}
        {uiState.sidebarTab === 'annotations' && (
          <AnnotationsPanel />
        )}
        {uiState.sidebarTab === 'layers' && (
          <LayersPanel />
        )}
        {uiState.sidebarTab === 'attachments' && (
          <AttachmentsPanel />
        )}
      </div>
    </div>
...
function ThumbnailsPanel() {
  ...
  const filteredPages = pdfState.activeDocument.pages.filter(page => 
      ...
      <div className="flex-1 overflow-y-auto p-3">
        <div className="grid grid-cols-2 gap-3">
          {filteredPages.map((page, index) => (
            <div
              key={page.id}
              onClick={() => pdfDispatch({ type: 'SET_CURRENT_PAGE', payload: page.pageNumber })}
              ...
              <div className="aspect-[3/4] bg-white rounded-xl p-2">
                ...
                <div className={`w-full h-full rounded-lg flex items-center justify-center transition-all ${
                  ...
                  <div className="text-center">
                    <DocumentIcon className={`w-8 h-8 mx-auto mb-1 ${
                      pdfState.activeDocument?.currentPage === page.pageNumber
                        ? 'text-blue-500'
      ...
      <div className="p-3 border-b border-gray-200 bg-white">
        ...
        <div className="flex space-x-1">
          {[
            ...
            { id: 'highlight', label: 'Highlights', count: filterCounts.highlights },
          ...
          <div className="flex items-center justify-center h-full p-6">
            <div className="text-center">
              <div className="bg-gradient-to-br from-green-50 to-emerald-50 rounded-xl p-6 border border-green-100">
                ...
                <p className="text-sm text-gray-600 mb-4">Add comments and annotations to collaborate on this document</p>
      ...
      <div className="flex-1 overflow-y-auto">
        {layers.length === 0 ? (
          ...
          <div className="p-3 space-y-2">
            {layers.map((layer) => (
              <div key={layer.id} className="bg-white border border-gray-200 rounded-lg p-3 hover:shadow-sm transition-shadow">
                <div className="flex items-center justify-between">
...
function AttachmentsPanel() {
  const [attachments] = useState([
    { id: 1, name: 'Document.pdf', size: '2.4 MB', type: 'pdf', dateAdded: new Date('2024-01-15') },
    { id: 2, name: 'Image.jpg', size: '1.2 MB', type: 'image', dateAdded: new Date('2024-01-14') },
    { id: 3, name: 'Spreadsheet.xlsx', size: '856 KB', type: 'excel', dateAdded: new Date('2024-01-13') },
  ])
        ...
        {attachments.length === 0 ? (
          ...
          <div className="p-3 space-y-3">
            {attachments.map((attachment) => (
              <div key={attachment.id} className="bg-white border border-gray-200 rounded-xl p-4 shadow-sm hover:shadow-md transition-shadow">
                ...
                <div className="text-xs text-gray-500">
                  Added {attachment.dateAdded.toLocaleDateString()}
                </div>
              </div>
            ))}
          </div>
