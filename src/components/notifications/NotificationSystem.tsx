              ...
              <div className="flex-1">
                ...
                {notification.metadata && (
                  <div className="text-xs text-gray-500 space-y-1">
                    {notification.metadata.documentId && <div>Document: {notification.metadata.documentId}</div>}
                    {notification.metadata.userId && <div>User: {notification.metadata.userId}</div>}
                    {notification.metadata.location && <div>Location: {notification.metadata.location}</div>}
                  </div>
                )}
              </div>
    ...
    <>
      ...
      <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
        <div className="bg-white rounded-lg shadow-xl max-w-4xl w-full mx-4 max-h-[90vh] overflow-hidden">
          ...
          {showSettings ? (
            <div className="p-6 max-h-[calc(90vh-120px)] overflow-y-auto">{renderSettings()}</div>
          ) : (
            <>
              {/* Tabs and Filters */}
                ...
                <div className="flex items-center justify-between px-6 py-4">
                  ...
                  <nav className="flex space-x-8">
                    {[
                      ...
                      >
                        <span>{tab.name}</span>
                        {tab.count > 0 && <Badge className="bg-gray-100 text-gray-600 text-xs">{tab.count}</Badge>}
                      </button>
                    ))}
                  </nav>
