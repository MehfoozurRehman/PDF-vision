      ...
      <div className="space-y-6">
                ...
                .map((issue) => (
                  <div key={issue.id} className="flex items-center justify-between p-3 bg-red-50 rounded-lg">
                    ...
                    <div className="text-right">
                      <Badge className="bg-red-100 text-red-800 text-xs mb-1">{issue.severity.toUpperCase()}</Badge>
                      <p className="text-xs text-gray-500">Page {issue.location.page}</p>
                    </div>
                  </div>
                ))}
            </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {ACCESSIBILITY_CATEGORIES.map((category) => {
            ...
            const categoryScore = categoryTests.length > 0 ? Math.round((passedCount / categoryTests.length) * 100) : 0;
                    ...
                    <div className="w-full bg-gray-200 rounded-full h-2">
                      <div
                        className={cn(
                          ...
                          categoryScore >= 80 ? "bg-green-600" : categoryScore >= 60 ? "bg-yellow-600" : "bg-red-600",
                        )}
              ...
              >
                <div className="space-y-4">
                  ...
                  <div className="text-center">
                    ...
                    <p className="text-sm text-gray-600">{Math.round((test.score / test.maxScore) * 100)}% Score</p>
                  </div>
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between mb-2">
                      <div className="flex items-center space-x-3">
                        ...
                        <Badge className={cn("text-xs", STATUS_COLORS[test.status])}>
                          <StatusIcon className="w-3 h-3 mr-1" />
                          {test.status.replace("_", " ").charAt(0).toUpperCase() +
                            test.status.replace("_", " ").slice(1)}
                        </Badge>
                        <Badge className="bg-blue-100 text-blue-800 text-xs">{test.level}</Badge>
                      </div>
                      ...
                      <div className="flex items-center space-x-4 text-sm">
                        <div className="text-right">
                          ...
                          <p className="text-xs text-gray-600">{Math.round((test.score / test.maxScore) * 100)}%</p>
                        </div>
                      </div>
                    </div>
      ...
      <div className="bg-white rounded-lg shadow-xl max-w-7xl w-full mx-4 max-h-[90vh] overflow-hidden">
        ...
        <div className="flex items-center justify-between p-6 border-b border-gray-200">
          <div className="flex items-center space-x-3">
            ...
            <Badge className="bg-blue-100 text-blue-800">{getComplianceScore()}% compliant</Badge>
          </div>
