...
export default function AnalyticsReporting({
  ...
  const [activeTab, setActiveTab] = useState<"overview" | "metrics" | "reports" | "activities" | "performance">(
    "overview",
  );
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <Card className="p-6">
            <div className="flex items-center justify-between mb-4">
              ...
              <Badge
                className={cn(
                  currentPerformance?.health.overall === "healthy"
                    ? "bg-green-100 text-green-800"
                    : currentPerformance?.health.overall === "warning"
                  ...
                  <div className="bg-gray-50 rounded-lg p-3">
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-sm font-medium">
                        {Math.round(
                          (currentPerformance.metrics.memory.used / currentPerformance.metrics.memory.total) * 100,
                        )}
                        %
                      </span>
                    </div>
                    <div className="w-full bg-gray-200 rounded-full h-2 mt-2">
                      <div
                        ...
                        style={{
                          width: `${(currentPerformance.metrics.memory.used / currentPerformance.metrics.memory.total) * 100}%`,
                        }}
                      />
                    </div>
                  </div>
              ...
              {recentActivities.map((activity) => {
                ...
                return (
                  <div key={activity.id} className="flex items-center space-x-3 p-3 bg-gray-50 rounded-lg">
                    <div className="p-2 bg-white rounded-lg">{getActionIcon(activity.action)}</div>
                    ...
                    <Badge className="bg-blue-100 text-blue-800 text-xs">{activity.device.type}</Badge>
                  </div>
                );
              })}
            </div>
          ...
          return (
            ...
            >
              <div className="space-y-4">
                ...
                {/* Last Updated */}
                <div className="text-xs text-gray-500">Updated {metric.lastUpdated.toLocaleString()}</div>
              </div>
            </Card>
          );
        })}
