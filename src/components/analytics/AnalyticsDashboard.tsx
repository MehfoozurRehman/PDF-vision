...
export default function AnalyticsDashboard({
  ...
  const [activeTab, setActiveTab] = useState<'overview' | 'usage' | 'documents' | 'users' | 'performance' | 'insights'>('overview')
          ...
          {activeTab === 'overview' && (
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                {renderMetricCard(
                  'Total Documents',
                  formatNumber(data.overview.totalDocuments),
                  DocumentIcon,
                  12.5
                )}
                {renderMetricCard(
                      ...
                      <div className="flex items-center space-x-2">
                        <div className="w-24 bg-gray-200 rounded-full h-2">
                          <div 
                            ...
                            style={{ width: formatPercentage(data.devices.desktop, data.devices.desktop + data.devices.mobile + data.devices.tablet) }}
                          />
                        </div>
                        <span className="text-sm text-gray-600 w-12 text-right">
                          {formatPercentage(data.devices.desktop, data.devices.desktop + data.devices.mobile + data.devices.tablet)}
                        </span>
                      </div>
                    </div>
                      ...
                      <div className="flex items-center space-x-2">
                        <div className="w-24 bg-gray-200 rounded-full h-2">
                          <div 
                            ...
                            style={{ width: formatPercentage(data.devices.mobile, data.devices.desktop + data.devices.mobile + data.devices.tablet) }}
                          />
                        </div>
                        <span className="text-sm text-gray-600 w-12 text-right">
                          {formatPercentage(data.devices.mobile, data.devices.desktop + data.devices.mobile + data.devices.tablet)}
                        </span>
                      </div>
                    </div>
                      ...
                      <div className="flex items-center space-x-2">
                        <div className="w-24 bg-gray-200 rounded-full h-2">
                          <div 
                            ...
                            style={{ width: formatPercentage(data.devices.tablet, data.devices.desktop + data.devices.mobile + data.devices.tablet) }}
                          />
                        </div>
                        <span className="text-sm text-gray-600 w-12 text-right">
                          {formatPercentage(data.devices.tablet, data.devices.desktop + data.devices.mobile + data.devices.tablet)}
                        </span>
                      </div>
                    </div>
                  </div>
          ...
          {activeTab === 'usage' && (
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                ...
                {renderMetricCard(
                  'Annotations',
                  formatNumber(data.overview.totalAnnotations),
                  PencilIcon,
                  18.4
                )}
              </div>
                          ...
                          <div className="flex items-center space-x-2">
                            ...
                            <div className={cn(
                              ...
                              <div 
                                ...
                                style={{ width: `${(annotation.count / Math.max(...data.features.annotations.map(a => a.count))) * 100}%` }}
                              />
                            </div>
                          </div>
                        </div>
          ...
          {activeTab === 'documents' && (
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                ...
                {renderMetricCard(
                  'Avg File Size',
                  formatBytes(data.documents.largestDocuments.reduce((acc, doc) => acc + doc.size, 0) / data.documents.largestDocuments.length),
                  DocumentIcon
                )}
                {renderMetricCard(
                  'Total Storage',
                  formatBytes(data.performance.storageUsed),
                  DocumentIcon
                )}
                {renderMetricCard(
                  'Storage Usage',
                      ...
                      <div key={doc.id} className="flex items-center justify-between">
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-xs text-gray-500">
                            Last viewed: {doc.lastViewed.toLocaleDateString()}
                          </p>
                        </div>
                        <div className="flex items-center space-x-2">
          ...
          {activeTab === 'users' && (
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                ...
                {renderMetricCard(
                  'New Users',
                  formatNumber(data.users.recentlyJoined.length),
                  UserGroupIcon,
                  12.7
                )}
              </div>
                    ...
                    {data.users.mostActive.slice(0, 5).map((user, index) => (
                      <div key={user.id} className="flex items-center justify-between">
                        ...
                        <div className="text-sm text-gray-600">
                          {formatNumber(user.actionsCount)} actions
                        </div>
                      </div>
                    ))}
                  </div>
          ...
          {activeTab === 'performance' && (
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                {renderMetricCard(
                  'Avg Load Time',
                  `${data.performance.averageLoadTime.toFixed(2)}s`,
                  ClockIcon,
                  -5.2
                )}
                {renderMetricCard(
                      ...
                      <div className="flex items-start space-x-3">
                        ...
                        <div>
                          ...
                          <p className="text-sm text-blue-700 mt-1">
                            Most activity occurs between 9-11 AM and 2-4 PM. Consider scheduling maintenance outside these windows.
                          </p>
                        </div>
                      </div>
                    </div>
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex items-start space-x-3 p-4 bg-gray-50 rounded-lg">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-gray-600 mt-1">
                        Documents with 3+ collaborators have 60% higher engagement. Promote team features more prominently.
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-start space-x-3 p-4 bg-gray-50 rounded-lg">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-gray-600 mt-1">
                        New users who complete the tutorial are 3x more likely to become active. Consider making it mandatory.
                      </p>
                    </div>
                  </div>
                </div>
