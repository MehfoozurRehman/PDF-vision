...
interface TemplateField {
  ...
  name: string
  label: string
  type: 'text' | 'number' | 'date' | 'email' | 'phone' | 'url' | 'textarea' | 'select' | 'checkbox' | 'radio' | 'file' | 'image' | 'signature' | 'table' | 'list' | 'formula' | 'barcode' | 'qrcode'
  required: boolean
  placeholder?: string
  defaultValue?: any
...
interface WorkflowAction {
  type: 'email' | 'webhook' | 'api_call' | 'file_generation' | 'data_export' | 'field_update' | 'redirect' | 'notification'
...
export default function TemplateManager({
  ...
  const filteredTemplates = templates.filter(template => {
    ...
    if (filter.searchQuery && !template.name.toLowerCase().includes(filter.searchQuery.toLowerCase()) &&
        !template.description.toLowerCase().includes(filter.searchQuery.toLowerCase()) &&
        !template.tags.some(tag => tag.toLowerCase().includes(filter.searchQuery.toLowerCase()))) return false
    return true
  })
