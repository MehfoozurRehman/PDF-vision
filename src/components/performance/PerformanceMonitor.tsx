...
export default function PerformanceMonitor({
  ...
  const filteredAlerts = alerts.filter(alert => {
    if (searchQuery && !alert.metricName.toLowerCase().includes(searchQuery.toLowerCase()) &&
                    ...
                    <div className="space-y-2">
                      <div className="flex items-center justify-between text-sm">
                        ...
                        <span className="font-medium">
                          {Math.round(((categoryMetrics.length - criticalCount - warningCount) / categoryMetrics.length) * 100)}%
                        </span>
                      </div>
      ...
      {viewMode === 'grid' ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredMetrics.map((metric) => {
            ...
            return (
              <Card key={metric.id} className="p-4 hover:shadow-lg transition-shadow cursor-pointer"
                    onClick={() => {
