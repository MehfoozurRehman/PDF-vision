...
interface WorkflowTrigger {
  id: string
  type: 'manual' | 'schedule' | 'webhook' | 'file_upload' | 'email' | 'form_submit' | 'api_call' | 'database_change' | 'file_change'
  name: string
  description: string
            ...
            <div>
              ...
              <p className="text-2xl font-semibold text-gray-900">
                {workflows.length > 0 ? Math.round(
                  workflows.reduce((sum, w) => sum + w.statistics.successRate, 0) / workflows.length
                ) : 0}%
              </p>
            </div>
          </div>
