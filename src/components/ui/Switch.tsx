    ...
    <div className={cn("flex items-start space-x-3", className)}>
      ...
      {(label || description) && (
        <div className="flex-1">
          {label && (
            <label
              className={cn("text-sm font-medium cursor-pointer", disabled ? "text-neutral-400" : "text-neutral-900")}
              onClick={handleToggle}
            >
              {label}
            </label>
          ...
          {description && (
            <p className={cn("text-sm", disabled ? "text-neutral-300" : "text-neutral-500")}>{description}</p>
          )}
        </div>
      )}
    </div>
