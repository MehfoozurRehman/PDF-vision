...
export default function DocumentProtection({
  ...
  const [settings, setSettings] = useState<ProtectionSettings>(
    currentProtection || defaultSettings
  )
  const [activeTab, setActiveTab] = useState<'password' | 'permissions' | 'watermark' | 'expiration' | 'access'>('password')
          ...
          <div>
            ...
            {settings.expiration.currentViews !== undefined && (
              <p className="text-xs text-neutral-500 mt-1">
                Current views: {settings.expiration.currentViews}
              </p>
            )}
          </div>
        </div>
        ...
        <div className="space-y-4 pl-4 border-l-2 border-primary-200">
          <div>
            ...
            <textarea
              ...
              onChange={(e) => updateSettings('accessControl', { allowedUsers: e.target.value.split('\n').filter(Boolean) })}
          ...
          <div>
            ...
            <textarea
              ...
              onChange={(e) => updateSettings('accessControl', { allowedDomains: e.target.value.split('\n').filter(Boolean) })}
