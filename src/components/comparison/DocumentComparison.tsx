...
export default function DocumentComparison({
  ...
  const filteredChanges = selectedComparison?.changes.filter(change => {
    ...
    if (filter.searchQuery && !change.description.toLowerCase().includes(filter.searchQuery.toLowerCase()) &&
        !change.content.before?.toLowerCase().includes(filter.searchQuery.toLowerCase()) &&
        !change.content.after?.toLowerCase().includes(filter.searchQuery.toLowerCase())) return false
    return true
  }) || []
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <Card className="p-6">
          ...
          <div className="space-y-4">
            ...
            >
              <option value="">Select document...</option>
              {documents.filter(doc => doc.id !== selectedDocA).map((doc) => (
                <option key={doc.id} value={doc.id}>
                  {doc.name} (v{doc.version})
                </option>
              ))}
            ...
            <div className="flex items-start justify-between mb-4">
              <div>
                ...
                <p className="text-sm text-gray-500">
                  Compared on {selectedComparison.createdAt.toLocaleDateString()}
                </p>
              </div>
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 gap-4">
            {comparisons.map((comparison) => (
              <Card key={comparison.id} className="p-4 cursor-pointer hover:shadow-md transition-shadow"
                ...
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    <div className="flex items-center space-x-3 mb-2">
                      ...
                      <Badge className={cn(
                        'text-xs',
                        comparison.status === 'completed' ? 'bg-green-100 text-green-700' :
                        comparison.status === 'processing' ? 'bg-blue-100 text-blue-700' :
