                ...
                <button
                  ...
                  onKeyDown={(e) => {
                    if (e.key === 'ArrowLeft' || e.key === 'ArrowRight') {
                      ...
                      // Focus the new tab
                      setTimeout(() => {
                        const newTab = document.querySelector(`[aria-controls="${tabs[nextIndex]}-panel"]`) as HTMLElement
                        newTab?.focus()
                      }, 0)
                    }
                  }}
                      ...
                      >
                        {Object.entries(qualitySettings).map(([quality, config]) => (
                          <option key={quality} value={quality}>
                            {quality.charAt(0).toUpperCase() + quality.slice(1)} ({config.dpi} DPI) - {config.description}
                          </option>
                        ))}
                      </select>
                    </div>
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-neutral-500">
                Quality: {qualitySettings[settings.quality].dpi} DPI • 
