...
const SharedReviewModal: React.FC = () => {
  ...
  const handleInviteReviewer = (sessionId: string, reviewer: Omit<Reviewer, "id" | "joinedAt">) => {
    ...
    setSessions((prev) =>
      prev.map((session) =>
        session.id === sessionId ? { ...session, reviewers: [...session.reviewers, newReviewer] } : session,
      ),
    );
  };
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl w-full max-w-6xl h-[90vh] flex flex-col">
        <div className="flex items-center justify-between p-4 border-b border-gray-200 dark:border-gray-700">
          <h2 className="text-xl font-semibold text-gray-900 dark:text-white">Shared Review</h2>
          <button
            onClick={handleClose}
