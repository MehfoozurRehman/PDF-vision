...
export default function ExportModal() {
      ...
      if (exportOptions.format === 'pdf') {
        ...
        if (exportOptions.compression || exportOptions.password) {
          const compressOptions = {
            optimizeLevel: exportOptions.quality === 'maximum' ? 4 as const : 
                          exportOptions.quality === 'high' ? 3 as const :
                          exportOptions.quality === 'medium' ? 2 as const : 1 as const,
        ...
        const conversionOptions: ConversionOptions = {
          ...
          dpi: exportOptions.quality === 'maximum' ? 300 :
               exportOptions.quality === 'high' ? 200 :
               exportOptions.quality === 'medium' ? 150 : 100,
          quality: exportOptions.quality === 'maximum' ? 100 :
                   exportOptions.quality === 'high' ? 85 :
                   exportOptions.quality === 'medium' ? 70 : 50,
  ...
  const getEstimatedSize = () => {
    ...
    const pageCount = exportOptions.pages === 'current' ? 1 : 
                     exportOptions.pages === 'all' ? pdfState.activeDocument.pages.length :
                     exportOptions.pages === 'range' ? 5 : // Estimate
      ...
      <div className="bg-white rounded-lg shadow-xl max-w-2xl w-full mx-4 max-h-[90vh] flex flex-col">
              ...
              <label className="flex items-center">
                ...
                {pdfState.activeDocument && (
                  <span className="ml-2 text-sm text-gray-500">
                    ({pdfState.activeDocument.pages.length} pages)
                  </span>
                )}
              </label>
              <label className="flex items-center">
        ...
        <div className="flex justify-between items-center p-6 border-t border-gray-200">
          <div className="text-sm text-gray-600">
            {pdfState.activeDocument && (
              <span>Ready to export {pdfState.activeDocument.name}</span>
            )}
          </div>
