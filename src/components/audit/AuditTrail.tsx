...
interface AuditEvent {
  id: string
  timestamp: Date
  eventType: 'access' | 'modification' | 'deletion' | 'creation' | 'permission_change' | 'security' | 'system' | 'compliance'
  action: string
  description: string
...
export default function AuditTrail({
  ...
  const renderComplianceTab = () => {
    ...
    const overallScore = Math.round(
      (complianceStats.compliant / (rules.length - complianceStats.notApplicable)) * 100
    )
    ...
    return (
      <div className="space-y-6">
        ...
        <Card className="p-6">
          ...
          <div className="space-y-4">
            {rules.filter(rule => !filters.regulation || rule.regulation === filters.regulation).map((rule) => {
